...
void loader_loadGeexOS()
{
	...
	Address physicalBaseAddress;
	size_t kernelImageLen;

	//TODO: read kernel location from a config-file
	if(!loadPeFile("hd(0,0)/system/gxoskrnl.exe", MemoryTypeGeexOSKernelExecutable, &physicalBaseAddress, &kernelImageLen))
	...
	entryPoint p = (entryPoint)0xC0001000;
	p();
}
