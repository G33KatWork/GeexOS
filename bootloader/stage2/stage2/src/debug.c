...
static DBGPORT enabledOutputPorts = DBGPORT_SCREEN | DBGPORT_RS232;
...
void debug_init()
{
	debug_enableOutputPort(DBGPORT_SCREEN | DBGPORT_RS232);
...
size_t debug_printf(const char* format, ...)
{
	va_list params;
   	...
   	while(*ptr)
   	{
   		if(enabledOutputPorts & DBGPORT_SCREEN)
	   		arch_screen_putchar(*ptr);

	   	if(enabledOutputPorts & DBGPORT_RS232)
