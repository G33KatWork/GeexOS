...
int kmain(MultibootInfo* multibootInformation, unsigned int magic) __attribute__((dllexport));
...
uint32_t mapElfFile(void* elfPointer)
{
    ...
    for(int i = 0; i < numHeaders; i++)
    {
        ...
        if(header->type == ELF_PROGRAM_HEADER_TYPE_LOAD)
        {
            ...
            //FIXME: fix this...
            if(!IS_PAGE_ALIGNED(header->vaddr))
                panic("vaddr of program header is not page aligned");
